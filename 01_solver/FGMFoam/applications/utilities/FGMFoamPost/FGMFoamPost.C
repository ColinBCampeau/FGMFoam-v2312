...
int main(int argc, char *argv[])
{
    ...
    if (args.found("fields")) //args.optionFound()
    {
...
//    const word combTypeName(readScalar("combustionModel",combProps));
//    const word combTypeName = word(combProps.lookup("combustionModel")); //

// Terms linked to useProgressVariableVariance are commented, since VarPV is not calculated in this version
// and the following lines, solely related to usePVV switch, give errors
 
/*    const word combTypeName = combProps.readEntry("combustionModel", combTypeName); 
...
//    scalar Yu2I, YuYbI, Yb2I;
    ...
    tableNames.push_uniq("T");   //--- Read 'T' table, push_uniq, avert. seulement pour PVtableNames, pas tableNames
    tableNames.push_uniq("SourcePV");
    Foam::combustionModels::tableSolver solver(Foam::combustionModels::tableSolver(mesh, tableNames));
    
    hashedWordList PVtableNames;
    PVtableNames.clear();
    PVtableNames.push_uniq("PVmin");
    PVtableNames.push_uniq("PVmax");
/*    if (useProgressVariableVariance_)
    {
      PVtableNames.push_uniq("Yu2I");
      PVtableNames.push_uniq("YuYbI");
      PVtableNames.push_uniq("Yb2I");
    }
*/
...
/*		   if (useProgressVariableVariance_) // uPVV SWITCH
		   {
       ...
       {
// not used          const fvPatchScalarField& pvarPV = varPV.boundaryField()[patchi];
